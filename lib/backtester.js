...
let runUpdate = function (days, selector) {
  let zenbot_root = path.resolve(__dirname, '..')
  let zenbot_cmd = process.platform === 'win32' ? path.join(zenbot_root, 'zenbot.bat') : path.join(zenbot_root, 'zenbot.sh')
...
module.exports = {
  ...
  runCommand: (taskStrategyName, phenotype, command, cb) => {
    ...
    const proc = spawn(cmdName, cmdArgs, { cwd: path.resolve(__dirname, '..') })
    var endData = ''
