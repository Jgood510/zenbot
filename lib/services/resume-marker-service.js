...
module.exports = function (conf) {
  ...
  theService.load = (cb) => {
    collectionServiceInstance.getResumeMarkers().find({selector: selector.normalized}).toArray().then(function (data) {
      ranges = data
  ...
  theService.flush = (cb) => {
    if (ranges.length > 0) {
      collectionServiceInstance.getResumeMarkers().deleteMany({selector: selector.normalized}).then(function () {
        if (ranges.length > 1) {
          collectionServiceInstance.getResumeMarkers().insertMany(ranges).then(function () {
            if (cb !== undefined)
              cb()
          }).catch(function (err) { throw err })
        } else {
          collectionServiceInstance.getResumeMarkers().insertOne(ranges[0]).then(function () {
            if (cb !== undefined)
              cb()
          }).catch(function (err) { throw err })
        }
